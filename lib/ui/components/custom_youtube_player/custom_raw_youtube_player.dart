...
class CustomRawYoutubePlayer extends StatefulWidget {
  ...
  @override
  State<CustomRawYoutubePlayer> createState() => _CustomRawYoutubePlayerState();
}

class _CustomRawYoutubePlayerState extends State<CustomRawYoutubePlayer>
    with WidgetsBindingObserver {
  CustomYoutubePlayerController? controller;
  ...
  Widget build(BuildContext context) {
    controller = CustomYoutubePlayerController.of(context);

  if (controller?.initialVideoId == null || controller!.initialVideoId.isEmpty) {
    return const Center(child: CircularProgressIndicator()); // Or SizedBox.shrink()
  }
        ...
        onWebViewCreated: (webController) {
          ...
          webController
            ...
            ..addJavaScriptHandler(
              handlerName: 'Errors',
              callback: (args) {
                print("Error occurred: ${args.first}");
                controller!.updateValue(
                  controller!.value.copyWith(errorCode: 404),
                );
              },
            )
            ..addJavaScriptHandler(
